...
import { upcomingSlots, SLOTS_PER_HOUR, parseSlotKey } from '../lib/dates';
...
export default async function HomePage({ searchParams }) {
  ...
  // If user came from the iPad substitute flow, pre-open the specific slot
  const prefilledSlot = params.slot ? parseSlotKey(params.slot) : null;
  const substituteMode = params.mode === 'substitute';
  ...
  return (
    <div className="page">
      ...
      {substituteMode && (
        <section className="guidelines" style={{ background: '#fef9ee', borderColor: '#d4a945' }}>
          <h2 style={{ color: '#6e4a0e' }}>
            Substitute Sign-Up
            <span className="lang-es">Registro de Sustituto</span>
          </h2>
          <p style={{ margin: '0 0 12px' }}>
            <strong>Thank you for offering to cover this hour.</strong> Please remember: the Blessed Sacrament must not be left alone. By signing up, you commit to staying for the <strong>full hour</strong>.
          </p>
          <p style={{ margin: 0, color: 'var(--ink-secondary)', fontStyle: 'italic', fontSize: 17 }}>
            Gracias por ofrecerse a cubrir esta hora. Recuerde: el Santísimo no puede quedar solo. Al inscribirse, se compromete a quedarse la <strong>hora completa</strong>.
          </p>
        </section>
      )}
